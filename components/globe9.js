...
function GlobeComponent() {
  ...
  const getObjectThreeObject = (d) => {
    ...
    if (d.isUserPoint) {
      ...
      color = 'brightred'; // Bright purple color for newly added satellites
    } else if (d.color == 'red') { // New condition for newly added satellites
      geometry = new THREE.BoxGeometry(
        SAT_SIZE*2 * globeRadius / EARTH_RADIUS_KM,
        SAT_SIZE*2 * globeRadius / EARTH_RADIUS_KM,
        SAT_SIZE*2 * globeRadius / EARTH_RADIUS_KM
      );
      color = 'red'; // Bright purple color for newly added satellites

    } else if (d.color == 'green') { // New condition for newly added satellites
      geometry = new THREE.BoxGeometry(
        SAT_SIZE * globeRadius / EARTH_RADIUS_KM,
        SAT_SIZE * globeRadius / EARTH_RADIUS_KM,
        SAT_SIZE * globeRadius / EARTH_RADIUS_KM
  ...
  const handleGlobeClick = async (event) => {
   ...
   /*const navSats = saty(lat, lng, 'nav', 5 + Math.floor(Math.random() * 6));  // 5-10 nav satellites
    ...
    const satName = window.prompt('Enter disaster/anomoly name:');
